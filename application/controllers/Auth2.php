...
class Auth2 extends CI_Controller {
        ...
        if(!$this->auth_ldap->is_authenticated()) {
            ...
            if($rules->run() && $this->auth_ldap->login(
                ...
                if($this->session->flashdata('tried_to')) {
                    ...
                    redirect('admin/home');
                    //Array ( [__ci_last_regenerate] => 1541050766 [username] => paphawin_pan [cn] => Paphawin Panyasai [role] => [logged_in] => 1 )
                }
            }else {
                // Login FAIL
                $this->load->view('auth2/login_form', array('login_fail_msg'
    ...
    function logout() {
        if($this->session->userdata('logged_in')) {
            ...
            $this->load->view('auth2/logout_view', $data);
    }
}
